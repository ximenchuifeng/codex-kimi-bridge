...
2. Codex delegates implementation to Kimi through `kimi-delegate`.
3. Kimi changes code and runs verification.
4. Codex reads handoff/diff, independently reviews, and either accepts or sends follow-up feedback.
...
`kimi_delegate_task` and `kimi_continue_task` return `webUrl`, which can be opened in Kimi Web to watch the session.
